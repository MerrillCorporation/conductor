...
public interface ConductorClientConfiguration {
     ...
     * If it is set to false, the payload is rejected and the workflow execution is marked as FAILED.
     */
     ...
     * beyond which the payload will be processed based on {@link ConductorClientConfiguration#isExternalPayloadStorageEnabled()}.
     * If it is set to true, the payload is stored in external location.
     * If it is set to false, the payload is rejected and the task execution is marked as FAILED_WITH_TERMINAL_ERROR
     */
    int getTaskOutputPayloadThresholdKB();

    /**
     * @return when set to true, enables the use of external storage for storing workflow/task
     * input and output JSON payloads with size greater than thresholds
     */
